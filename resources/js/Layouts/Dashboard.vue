...
import DashboardNavigation from '@/Layouts/Components/DashboardNavigation.vue'
import DashboardProfile from '@/Layouts/Components/DashboardProfile.vue'
            ...
            <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
                    ...
                    <div class="border-b border-gray-700">
                        <div class="flex h-16 items-center justify-between px-4 sm:px-0">
                            <div class="flex items-center">
                                ...
                                <!-- Navigation -->
                                <DashboardNavigation :navigation="navigation" :pages="pages"
                                    :shop-navigation="shopNavigation" />
                            </div>
                            <!-- Profile -->
                            <DashboardProfile :user-navigation="userNavigation" />
                ...
                <DisclosurePanel class="border-b border-gray-700 md:hidden">
                    <div class="space-y-1 px-2 py-3 sm:px-3">
                        <DisclosureButton v-for="item in pages" :key="item.name" as="a" :href="item.href"
                    ...
                    <div class="border-t border-gray-700 pb-3 pt-4">
                        <div class="flex items-center px-5">
                            <div class="flex-shrink-0">
                                <img class="h-10 w-10 rounded-full" :src="usePage().props?.auth?.user.profile_photo_url"
                                    alt="" />
                            </div>
                            <div class="ml-3">
                                <div class="text-base font-medium leading-none text-white">{{
                                    usePage().props?.auth?.user.username }}</div>
                                <div class="text-sm font-medium leading-none text-gray-400">{{
                                    usePage().props?.auth?.user.email }}</div>
                            </div>
                        </div>
                        <div class="mt-3 space-y-1 px-2">
                            <DisclosureButton v-for="item in userNavigation" @click="item.click" :key="item.name" as="a"
                                :href="item.href"
