        ...
        <div class="flex items-center justify-end">
            <div class="relative mt-2 rounded-md shadow-sm flex items-center">
                ...
                <input type="text" v-model="search" name="search" id="search"
                    ...
                    placeholder="Search" />
                <XMarkIcon class="h-5 w-5 text-gray-400 absolute right-0 mr-3 cursor-pointer" aria-hidden="true"
                    @click="resetSearch" v-if="search" />
            </div>
        </div>
                        ...
                        <table class="min-w-full border-separate border-spacing-0">
                            ...
                            <tbody>
                                <tr v-for="(item, index) in items.data">
                                    ...
                                    <td
                                        :class="['relative whitespace-nowrap py-4 pr-4 pl-3 text-right text-sm font-medium sm:pr-8 lg:pr-8']">
                                        <a v-if="!item.is_in_shop" :href="route('item.create', item.Index)"
                                            class="text-indigo-600 hover:text-indigo-900">Add to Shop</a>
                                        <a v-else :href="route('item.create', item.Index)"
                                            class="text-indigo-600 hover:text-indigo-900">Edit Item in Shop</a>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
